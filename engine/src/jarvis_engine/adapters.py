...
class ImageAdapter(AdapterBase):
    ...
    def __init__(self, repo_root: Path) -> None:
        ...
        self.script = Path(
            os.getenv(
                "JARVIS_IMAGE_SCRIPT",
                r"C:\Users\Conner\.codex\skills\imagegen\scripts\image_gen.py",
            )
        )
...
class VideoAdapter(AdapterBase):
    ...
    def __init__(self, repo_root: Path) -> None:
        ...
        self.script = Path(
            os.getenv(
                "JARVIS_SORA_SCRIPT",
                r"C:\Users\Conner\.codex\skills\sora\scripts\sora.py",
            )
        )
...
def _build_sphere_obj(prompt: str, rings: int, segments: int) -> tuple[str, int, int]:
    ...
    lines = [f"# Generated by Jarvis local 3D adapter", f"# prompt: {comment}", "o jarvis_sphere"]
...
def _build_cylinder_obj(prompt: str, segments: int) -> tuple[str, int, int]:
    ...
    lines = [f"# Generated by Jarvis local 3D adapter", f"# prompt: {comment}", "o jarvis_cylinder"]
    lines.extend([f"v {x:.6f} {y:.6f} {z:.6f}" for x, y, z in vertices])
    lines.extend([f"f {a} {b} {c}" for a, b, c in faces])
    return "\n".join(lines) + "\n", len(vertices), len(faces)
