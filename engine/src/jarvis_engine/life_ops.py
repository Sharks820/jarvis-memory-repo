...
def build_daily_brief(snapshot: OpsSnapshot) -> str:
    ...
    costly_subs = [
        ...
        if safe_float(s.get("monthly_cost", 0.0)) >= 20.0
    ]
    ...
    for b in snapshot.bills:
        ...
        if status in {"due", "overdue"}:
            actions.append(
                ...
                f"(amount ${safe_float(b.get('amount', 0.0)):.2f})"
            )

    for s in snapshot.subscriptions:
        monthly = safe_float(s.get("monthly_cost", 0.0))
        usage = safe_float(s.get("usage_score", 1.0), default=1.0)
...
def export_actions_json(actions: list[str], path: Path) -> None:
    ...
    atomic_write_json(path, records, secure=False)
    ...
    if snapshot.bills:
        ...
        if due_bills:
            ...
            for b in due_bills[:8]:
                ...
                amount = safe_float(b.get("amount", 0.0))
