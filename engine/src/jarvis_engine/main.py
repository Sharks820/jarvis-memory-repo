...
def cmd_gaming_mode(enable: bool | None, reason: str, auto_detect: str) -> int:
    result = _get_bus().dispatch(
        GamingModeCommand(enable=enable, reason=reason, auto_detect=auto_detect)
    )
    state = result.state
    print("gaming_mode")
...
def cmd_status() -> int:
    ...
    print(
        f"response=Engine status: {result.profile} profile, {result.operation_mode} mode, "
...
def cmd_ingest(source: str, kind: str, task_id: str, content: str) -> int:
    result = _get_bus().dispatch(
        IngestCommand(source=source, kind=kind, task_id=task_id, content=content)
    )
    print(
        f"ingested: id={result.record_id} source={result.source} kind={result.kind} task_id={result.task_id}"
    )
    return 0


def cmd_serve_mobile(
    host: str,
    port: int,
    token: str | None,
    ...
    if not effective_signing_key:
        print(
            "error: missing signing key. pass --signing-key or set JARVIS_MOBILE_SIGNING_KEY"
        )
        return 2
        ...
        try:
            ...
            if _created_utc:
                _created_dt = datetime.fromisoformat(
                    _created_utc.replace("Z", "+00:00")
                )
                _now_utc = (
                    datetime.now(tz=_created_dt.tzinfo)
                    if _created_dt.tzinfo
...
def cmd_memory_snapshot(create: bool, verify_path: str | None, note: str) -> int:
    result = _get_bus().dispatch(
        MemorySnapshotCommand(create=create, verify_path=verify_path, note=note)
    )
    if result.created:
...
def cmd_memory_maintenance(keep_recent: int, snapshot_note: str) -> int:
    result = _get_bus().dispatch(
        MemoryMaintenanceCommand(keep_recent=keep_recent, snapshot_note=snapshot_note)
    )
...
def cmd_owner_guard(
    *,
    ...
    print(f"updated_utc={state.get('updated_utc', '')}")
    print(
        "effect=voice_run_restricted_to_owner_and_mobile_api_restricted_to_trusted_devices_when_enabled"
    )
    return 0
...
def cmd_connect_grant(connector_id: str, scopes: list[str]) -> int:
    result = _get_bus().dispatch(
        ConnectGrantCommand(connector_id=connector_id, scopes=scopes)
    )
    if result.return_code != 0:
...
def cmd_phone_action(
    action: str, number: str, message: str, queue_path: Path, queue_action: bool = True
) -> int:
    result = _get_bus().dispatch(
        PhoneActionCommand(
            action=action,
...
def cmd_web_research(
    query: str, *, max_results: int, max_pages: int, auto_ingest: bool
) -> int:
    cleaned = query.strip()
    if not cleaned:
    ...
    if summary_parts:
        print(
            "response="
            + escape_response("Here's what I found: " + " | ".join(summary_parts))
        )
    else:
        ...
        print(
            "response="
            + escape_response(
                f"I searched the web for '{_query}' but couldn't find clear results."
            )
        )
...
def cmd_self_heal(
    *, force_maintenance: bool, keep_recent: int, snapshot_note: str, as_json: bool
) -> int:
    bus_result, _ = _dispatch(
        SelfHealCommand(
    ...
    *,
    ...
    result = _get_bus().dispatch(
        DaemonRunCommand(
            ...
            sync_every_cycles=sync_every_cycles,
...
def cmd_voice_enroll(user_id: str, wav_path: str, replace: bool) -> int:
    result = _get_bus().dispatch(
        VoiceEnrollCommand(user_id=user_id, wav_path=wav_path, replace=replace)
    )
...
def cmd_voice_verify(user_id: str, wav_path: str, threshold: float) -> int:
    result = _get_bus().dispatch(
        VoiceVerifyCommand(user_id=user_id, wav_path=wav_path, threshold=threshold)
    )
    if result.message.startswith("error:"):
        print(result.message)
        return 2
...
def cmd_voice_listen(
    ...
    _emit_voice_listen_state(
        "arming",
        details={"duration_s": duration, "language": language, "execute": execute},
    )
    _emit_voice_listen_state(
        "listening", details={"duration_s": duration, "language": language}
    )
    ...
    _emit_voice_listen_state(
        "processing", details={"duration_s": result.duration_seconds}
    )
    ...
    if execute and result.text:
        _emit_voice_listen_state(
            "executing", details={"transcription_chars": len(result.text)}
        )
    ...
    _emit_voice_listen_state(
        "idle",
        details={"reason": "transcription_complete", "confidence": result.confidence},
    )
    return 0
...
def cmd_self_test(threshold: float) -> int:
    ...
    for task_score in result.per_task_scores:
        print(
            f"  task={task_score.get('task_id', '?')} score={task_score.get('score', 0.0):.4f}"
        )
    print(f"message={result.message}")
    return 0
...
def _dispatch_serve_mobile(a) -> int:
    ...
    return cmd_serve_mobile(
        ...
        allow_insecure_bind=a.allow_insecure_bind,
...
def _dispatch_growth_eval(a) -> int:
    ...
    return cmd_growth_eval(
        model=a.model,
        endpoint=a.endpoint,
        tasks_path=Path(a.tasks_path),
        history_path=Path(a.history_path),
        num_predict=a.num_predict,
        temperature=a.temperature,
        think=think_opt,
        accept_thinking=a.accept_thinking,
...
def _dispatch_owner_guard(a) -> int:
    ...
    return cmd_owner_guard(
        ...
        trust_device=a.trust_device,
        revoke_device=a.revoke_device,
        set_master_password_value=os.getenv("JARVIS_MASTER_PASSWORD", "").strip()
        or a.set_master_password,
...
def _dispatch_gaming_mode(a) -> int:
    ...
    if a.enable:
        ...
        enable_opt = False
    return cmd_gaming_mode(
        enable=enable_opt, reason=a.reason, auto_detect=a.auto_detect
    )
...
def _dispatch_voice_run(a) -> int:
    ...
    return cmd_voice_run(
        text=a.text,
        execute=a.execute,
        approve_privileged=a.approve_privileged,
        speak=a.speak,
        snapshot_path=Path(a.snapshot_path),
        actions_path=Path(a.actions_path),
        voice_user=a.voice_user,
        voice_auth_wav=a.voice_auth_wav,
        voice_threshold=a.voice_threshold,
        master_password=os.getenv("JARVIS_MASTER_PASSWORD", "").strip()
        or a.master_password,
...
def main() -> int:
    ...
    sub.add_parser("status", help="Show engine bootstrap status.").set_defaults(
        handler=lambda a: cmd_status()
    )
    ...
    p_ingest.set_defaults(
        handler=lambda a: cmd_ingest(
            source=a.source, kind=a.kind, task_id=a.task_id, content=a.content
        )
    )
    ...
    p_mobile.add_argument(
        "--token", help="Shared token. Falls back to JARVIS_MOBILE_TOKEN env var."
    )
    p_mobile.add_argument(
        "--signing-key",
    ...
    p_route.set_defaults(
        handler=lambda a: cmd_route(risk=a.risk, complexity=a.complexity)
    )

    p_growth_eval = sub.add_parser(
        "growth-eval", help="Run golden-task model growth evaluation."
    )
    ...
    p_growth_report = sub.add_parser(
        "growth-report", help="Show growth trend from eval history."
    )
    ...
    p_growth_report.set_defaults(
        handler=lambda a: cmd_growth_report(
            history_path=Path(a.history_path), last=a.last
        )
    )

    p_growth_audit = sub.add_parser(
        "growth-audit", help="Show auditable prompt/response evidence."
    )
    ...
    p_growth_audit.set_defaults(
        handler=lambda a: cmd_growth_audit(
            history_path=Path(a.history_path), run_index=a.run_index
        )
    )
    ...
    p_intelligence.add_argument(
        "--output-path",
        default=str(repo_root() / ".planning" / "intelligence_dashboard.json"),
    )
    p_intelligence.add_argument(
        "--json", action="store_true", help="Print full JSON payload."
    )
    p_intelligence.set_defaults(
        handler=lambda a: cmd_intelligence_dashboard(
            last_runs=a.last_runs, output_path=a.output_path, as_json=a.json
        )
    )

    p_brain_status = sub.add_parser(
        "brain-status", help="Show high-level brain memory branch stats."
    )
    ...
    p_brain_context.set_defaults(
        handler=lambda a: cmd_brain_context(
            query=a.query, max_items=a.max_items, max_chars=a.max_chars, as_json=a.json
        )
    )

    p_brain_compact = sub.add_parser(
        "brain-compact", help="Compact old brain records into summary groups."
    )
    p_brain_compact.add_argument("--keep-recent", type=int, default=1800)
    p_brain_compact.add_argument("--json", action="store_true")
    p_brain_compact.set_defaults(
        handler=lambda a: cmd_brain_compact(keep_recent=a.keep_recent, as_json=a.json)
    )

    p_brain_regression = sub.add_parser(
        "brain-regression", help="Run anti-regression health checks for brain memory."
    )
    p_brain_regression.add_argument("--json", action="store_true")
    p_brain_regression.set_defaults(
        handler=lambda a: cmd_brain_regression(as_json=a.json)
    )

    p_kg_status = sub.add_parser(
        "knowledge-status",
        help="Show knowledge graph node/edge/locked/contradiction counts.",
    )
    ...
    p_clist = sub.add_parser(
        "contradiction-list", help="List knowledge graph contradictions."
    )
    p_clist.add_argument(
        "--status",
        default="pending",
        help="Filter by status (pending, resolved, or empty for all).",
    )
    p_clist.add_argument("--limit", type=int, default=20)
    p_clist.add_argument("--json", action="store_true")
    p_clist.set_defaults(
        handler=lambda a: cmd_contradiction_list(
            status=a.status, limit=a.limit, as_json=a.json
        )
    )

    p_cresolve = sub.add_parser(
        "contradiction-resolve", help="Resolve a knowledge graph contradiction."
    )
    p_cresolve.add_argument(
        "contradiction_id", type=int, help="Contradiction ID to resolve."
    )
    p_cresolve.add_argument(
        "--resolution", required=True, choices=["accept_new", "keep_old", "merge"]
    )
    p_cresolve.add_argument(
        "--merge-value",
        default="",
        help="Merged value (required for merge resolution).",
    )
    p_cresolve.set_defaults(
        handler=lambda a: cmd_contradiction_resolve(
            contradiction_id=a.contradiction_id,
            resolution=a.resolution,
            merge_value=a.merge_value,
        )
    )

    p_flock = sub.add_parser(
        "fact-lock", help="Lock or unlock a knowledge graph fact node."
    )
    p_flock.add_argument("node_id", help="Node ID to lock or unlock.")
    p_flock.add_argument("--action", default="lock", choices=["lock", "unlock"])
    p_flock.set_defaults(
        handler=lambda a: cmd_fact_lock(node_id=a.node_id, action=a.action)
    )

    p_kg_regression = sub.add_parser(
        "knowledge-regression", help="Run knowledge graph regression check."
    )
    p_kg_regression.add_argument(
        "--snapshot", default="", help="Path to previous snapshot metadata JSON."
    )
    p_kg_regression.add_argument("--json", action="store_true")
    p_kg_regression.set_defaults(
        handler=lambda a: cmd_knowledge_regression(
            snapshot_path=a.snapshot, as_json=a.json
        )
    )

    p_snapshot = sub.add_parser(
        "memory-snapshot", help="Create or verify signed memory snapshot."
    )
    ...
    p_snapshot.set_defaults(
        handler=lambda a: cmd_memory_snapshot(
            create=a.create, verify_path=a.verify_path, note=a.note
        )
    )

    p_maintenance = sub.add_parser(
        "memory-maintenance",
        help="Run compact + regression + signed snapshot maintenance.",
    )
    p_maintenance.add_argument("--keep-recent", type=int, default=1800)
    p_maintenance.add_argument("--snapshot-note", default="nightly")
    p_maintenance.set_defaults(
        handler=lambda a: cmd_memory_maintenance(
            keep_recent=a.keep_recent, snapshot_note=a.snapshot_note
        )
    )

    p_web_research = sub.add_parser(
        "web-research",
        help="Search the public web and summarize findings with source links.",
    )
    ...
    p_web_research.set_defaults(
        handler=lambda a: cmd_web_research(
            query=a.query,
            max_results=a.max_results,
            max_pages=a.max_pages,
            auto_ingest=not a.no_ingest,
        )
    )

    p_sync = sub.add_parser(
        "mobile-desktop-sync",
        help="Run cross-device state checks and write sync report.",
    )
    p_sync.add_argument("--json", action="store_true")
    p_sync.add_argument("--no-ingest", action="store_true")
    p_sync.set_defaults(
        handler=lambda a: cmd_mobile_desktop_sync(
            auto_ingest=not a.no_ingest, as_json=a.json
        )
    )

    p_self_heal = sub.add_parser(
        "self-heal", help="Run Jarvis self-healing checks and safe repairs."
    )
    ...
    p_self_heal.set_defaults(
        handler=lambda a: cmd_self_heal(
            force_maintenance=a.force_maintenance,
            keep_recent=a.keep_recent,
            snapshot_note=a.snapshot_note,
            as_json=a.json,
        )
    )

    p_persona = sub.add_parser(
        "persona-config", help="Configure Jarvis personality response style."
    )
    ...
    p_persona.set_defaults(
        handler=lambda a: cmd_persona_config(
            enable=a.enable,
            disable=a.disable,
            humor_level=a.humor_level,
            mode=a.mode,
            style=a.style,
        )
    )
    ...
    sub.add_parser(
        "migrate-memory", help="Migrate JSONL/JSON memory data into SQLite (one-time)."
    ).set_defaults(handler=lambda a: cmd_migrate_memory())

    sub.add_parser(
        "desktop-widget", help="Launch desktop-native Jarvis widget window."
    ).set_defaults(handler=lambda a: cmd_desktop_widget())

    p_run_task = sub.add_parser("run-task", help="Run multimodal Jarvis task.")
    p_run_task.add_argument(
        "--type", required=True, choices=["image", "code", "video", "model3d"]
    )
    p_run_task.add_argument("--prompt", required=True)
    p_run_task.add_argument(
        "--execute", action="store_true", help="Execute instead of dry-run plan."
    )
    p_run_task.add_argument(
        "--approve-privileged",
        action="store_true",
    ...
    p_run_task.set_defaults(
        handler=lambda a: cmd_run_task(
            task_type=a.type,
            prompt=a.prompt,
            execute=a.execute,
            approve_privileged=a.approve_privileged,
    ...
    p_ops_brief = sub.add_parser(
        "ops-brief", help="Generate daily life operations brief."
    )
    ...
    p_ops_brief.set_defaults(
        handler=lambda a: cmd_ops_brief(
            snapshot_path=Path(a.snapshot_path),
            output_path=Path(a.output_path) if a.output_path else None,
        )
    )

    p_ops_actions = sub.add_parser(
        "ops-export-actions", help="Export suggested actions from ops snapshot."
    )
    ...
    p_ops_actions.set_defaults(
        handler=lambda a: cmd_ops_export_actions(
            snapshot_path=Path(a.snapshot_path), actions_path=Path(a.actions_path)
        )
    )

    p_ops_sync = sub.add_parser(
        "ops-sync", help="Build live operations snapshot from connectors."
    )
    p_ops_sync.add_argument(
        "--output-path",
        default=str(repo_root() / ".planning" / _OPS_SNAPSHOT_FILENAME),
    )
    p_ops_sync.set_defaults(
        handler=lambda a: cmd_ops_sync(output_path=Path(a.output_path))
    )

    p_ops_autopilot = sub.add_parser(
        "ops-autopilot",
        help="Run connector check, sync, brief, action export, and automation.",
    )
    ...
    p_ops_autopilot.set_defaults(
        handler=lambda a: cmd_ops_autopilot(
            snapshot_path=Path(a.snapshot_path),
            actions_path=Path(a.actions_path),
            execute=a.execute,
            approve_privileged=a.approve_privileged,
            auto_open_connectors=a.auto_open_connectors,
        )
    )

    p_daemon = sub.add_parser(
        "daemon-run", help="Run Jarvis autopilot loop continuously."
    )
    ...
    p_daemon.add_argument(
        "--max-cycles", type=int, default=0, help="For testing; 0 means run forever."
    )
    p_daemon.add_argument(
        "--skip-missions",
        action="store_true",
        help="Disable background learning mission execution.",
    )
    ...
    p_daemon.set_defaults(
        handler=lambda a: cmd_daemon_run(
            ...
            idle_interval_s=a.idle_interval_s,
            idle_after_s=a.idle_after_s,
            run_missions=not a.skip_missions,
            sync_every_cycles=a.sync_every_cycles,
            self_heal_every_cycles=a.self_heal_every_cycles,
            self_test_every_cycles=a.self_test_every_cycles,
        )
    )

    p_mission_create = sub.add_parser(
        "mission-create", help="Create a learning mission."
    )
    ...
    p_mission_create.set_defaults(
        handler=lambda a: cmd_mission_create(
            topic=a.topic, objective=a.objective, sources=list(a.source)
        )
    )

    p_mission_status = sub.add_parser(
        "mission-status", help="Show recent learning missions."
    )
    ...
    p_mission_run = sub.add_parser(
        "mission-run", help="Run one learning mission with source verification."
    )
    p_mission_run.add_argument(
        "--id", required=True, help="Mission id from mission-create."
    )
    p_mission_run.add_argument("--max-results", type=int, default=8)
    p_mission_run.add_argument("--max-pages", type=int, default=12)
    p_mission_run.add_argument(
        "--no-ingest", action="store_true", help="Do not ingest verified findings."
    )
    p_mission_run.set_defaults(
        handler=lambda a: cmd_mission_run(
            mission_id=a.id,
            max_results=a.max_results,
            max_pages=a.max_pages,
            auto_ingest=not a.no_ingest,
    ...
    p_mission_cancel = sub.add_parser(
        "mission-cancel", help="Cancel a pending learning mission."
    )
    ...
    p_consolidate = sub.add_parser(
        "consolidate", help="Consolidate episodic memories into semantic facts."
    )
    p_consolidate.add_argument(
        "--branch", default="", help="Restrict to specific branch (empty = all)."
    )
    p_consolidate.add_argument(
        "--max-groups", type=int, default=20, help="Max groups to process."
    )
    p_consolidate.add_argument(
        "--dry-run", action="store_true", help="Compute clusters but don't write."
    )
    p_consolidate.set_defaults(
        handler=lambda a: cmd_consolidate(
            branch=a.branch, max_groups=a.max_groups, dry_run=a.dry_run
        )
    )

    p_runtime = sub.add_parser(
        "runtime-control", help="Pause/resume daemon and toggle safe mode."
    )
    ...
    p_runtime.set_defaults(
        handler=lambda a: cmd_runtime_control(
            pause=a.pause,
            resume=a.resume,
            safe_on=a.safe_on,
            safe_off=a.safe_off,
    ...
    p_owner = sub.add_parser(
        "owner-guard", help="Lock Jarvis to owner voice and trusted mobile devices."
    )
    ...
    p_gaming = sub.add_parser(
        "gaming-mode", help="Enable/disable low-impact mode for gaming sessions."
    )
    ...
    p_automation = sub.add_parser(
        "automation-run", help="Run planned actions with capability gates."
    )
    ...
    p_automation.set_defaults(
        handler=lambda a: cmd_automation_run(
            actions_path=Path(a.actions_path),
            approve_privileged=a.approve_privileged,
            execute=a.execute,
        )
    ...
    sub.add_parser(
        "connect-status", help="Show connector readiness and prompts."
    ).set_defaults(handler=lambda a: cmd_connect_status())

    p_connect_grant = sub.add_parser(
        "connect-grant", help="Grant connector permission."
    )
    p_connect_grant.add_argument(
        "--id", required=True, help="Connector id (for example: email, calendar)."
    )
    p_connect_grant.add_argument(
        "--scope", action="append", default=[], help="Optional scope (repeatable)."
    )
    p_connect_grant.set_defaults(
        handler=lambda a: cmd_connect_grant(connector_id=a.id, scopes=list(a.scope))
    )

    p_connect_bootstrap = sub.add_parser(
        "connect-bootstrap",
        help="Show connector prompts and optionally open setup links.",
    )
    p_connect_bootstrap.add_argument(
        "--auto-open", action="store_true", help="Open tap URLs in browser."
    )
    p_connect_bootstrap.set_defaults(
        handler=lambda a: cmd_connect_bootstrap(auto_open=a.auto_open)
    )

    p_phone_action = sub.add_parser(
        "phone-action", help="Queue phone action (send SMS/place call/ignore/block)."
    )
    ...
    p_phone_action.set_defaults(
        handler=lambda a: cmd_phone_action(
            action=a.action,
            number=a.number,
            message=a.message,
            queue_path=Path(a.queue_path),
        )
    )

    p_phone_spam = sub.add_parser(
        "phone-spam-guard", help="Analyze call logs and queue spam-block actions."
    )
    ...
    p_phone_spam.set_defaults(
        handler=lambda a: cmd_phone_spam_guard(
            call_log_path=Path(a.call_log_path),
    ...
    sub.add_parser(
        "voice-list", help="List available local Windows voices."
    ).set_defaults(handler=lambda a: cmd_voice_list())

    p_voice = sub.add_parser(
        "voice-say", help="Speak text with local Windows voice synthesis."
    )
    p_voice.add_argument("--text", required=True)
    p_voice.add_argument(
        "--profile", default="jarvis_like", choices=["jarvis_like", "default"]
    )
    ...
    p_voice.set_defaults(
        handler=lambda a: cmd_voice_say(
            text=a.text,
            profile=a.profile,
            voice_pattern=a.voice_pattern,
            output_wav=a.output_wav,
            rate=a.rate,
        )
    )

    p_voice_enroll = sub.add_parser(
        "voice-enroll", help="Enroll a user voiceprint from WAV."
    )
    p_voice_enroll.add_argument(
        "--user-id", required=True, help="Identity label, e.g. conner."
    )
    p_voice_enroll.add_argument(
        "--wav", required=True, help="Path to WAV sample of your voice."
    )
    p_voice_enroll.add_argument(
        "--replace", action="store_true", help="Replace existing profile."
    )
    p_voice_enroll.set_defaults(
        handler=lambda a: cmd_voice_enroll(
            user_id=a.user_id, wav_path=a.wav, replace=a.replace
        )
    )

    p_voice_verify = sub.add_parser(
        "voice-verify", help="Verify WAV sample against enrolled voiceprint."
    )
    ...
    p_voice_verify.set_defaults(
        handler=lambda a: cmd_voice_verify(
            user_id=a.user_id, wav_path=a.wav, threshold=a.threshold
        )
    )

    p_voice_run = sub.add_parser(
        "voice-run", help="Run a voice/text command through intent mapping."
    )
    ...
    p_voice_run.add_argument(
        "--speak", action="store_true", help="Speak completion status."
    )
    p_voice_run.add_argument("--voice-user", default="conner")
    p_voice_run.add_argument(
        "--voice-auth-wav",
        default="",
        help="Optional WAV path for voice authentication.",
    )
    ...
    p_voice_listen = sub.add_parser(
        "voice-listen", help="Record from microphone and transcribe speech-to-text."
    )
    p_voice_listen.add_argument(
        "--duration",
        type=float,
        default=30.0,
    ...
    p_voice_listen.add_argument(
        "--language", default="en", help="Language code hint for transcription."
    )
    p_voice_listen.add_argument(
        "--execute",
        action="store_true",
        help="Execute transcribed text as a voice command.",
    )
    p_voice_listen.set_defaults(
        handler=lambda a: cmd_voice_listen(
            duration=a.duration, language=a.language, execute=a.execute
        )
    )

    # -- Harvesting --
    p_harvest = sub.add_parser(
        "harvest", help="Harvest knowledge about a topic from external AI sources."
    )
    p_harvest.add_argument(
        "--topic", required=True, help="Topic to harvest knowledge about."
    )
    p_harvest.add_argument(
        "--providers",
        default=None,
        help="Comma-separated list of providers (default: all available).",
    )
    p_harvest.add_argument(
        "--max-tokens", type=int, default=2048, help="Max tokens per provider response."
    )
    p_harvest.set_defaults(
        handler=lambda a: cmd_harvest(
            topic=a.topic, providers=a.providers, max_tokens=a.max_tokens
        )
    )

    p_ingest_session = sub.add_parser(
        "ingest-session",
        help="Ingest knowledge from Claude Code or Codex session files.",
    )
    p_ingest_session.add_argument(
        "--source",
        required=True,
    ...
    p_ingest_session.add_argument(
        "--session-path", default=None, help="Specific session file path (optional)."
    )
    p_ingest_session.add_argument(
        "--project-path",
        default=None,
        help="Claude Code project path to scope search (optional).",
    )
    p_ingest_session.set_defaults(
        handler=lambda a: cmd_ingest_session(
            source=a.source, session_path=a.session_path, project_path=a.project_path
        )
    )

    p_harvest_budget = sub.add_parser(
        "harvest-budget", help="View or set harvest budget limits."
    )
    p_harvest_budget.add_argument(
        "--action", default="status", choices=["status", "set"], help="Budget action."
    )
    p_harvest_budget.add_argument("--provider", default=None, help="Provider name.")
    p_harvest_budget.add_argument(
        "--period", default=None, choices=["daily", "monthly"], help="Budget period."
    )
    p_harvest_budget.add_argument(
        "--limit-usd", type=float, default=None, help="USD limit."
    )
    p_harvest_budget.add_argument(
        "--limit-requests", type=int, default=None, help="Request count limit."
    )
    p_harvest_budget.set_defaults(
        handler=lambda a: cmd_harvest_budget(
            action=a.action,
            provider=a.provider,
            period=a.period,
    ...
    p_learn.add_argument(
        "--assistant-response", required=True, help="Assistant response text."
    )
    p_learn.set_defaults(
        handler=lambda a: cmd_learn(
            user_message=a.user_message, assistant_response=a.assistant_response
        )
    )

    p_cbq = sub.add_parser(
        "cross-branch-query", help="Query across knowledge branches."
    )
    ...
    sub.add_parser(
        "flag-expired", help="Flag expired knowledge graph facts."
    ).set_defaults(handler=lambda a: cmd_flag_expired())

    sub.add_parser(
        "memory-eval", help="Run memory-recall golden task evaluation."
    ).set_defaults(handler=lambda a: cmd_memory_eval())

    p_proactive = sub.add_parser(
        "proactive-check", help="Manually trigger proactive evaluation."
    )
    p_proactive.add_argument(
        "--snapshot-path", default="", help="Path to ops snapshot JSON."
    )
    p_proactive.set_defaults(
        handler=lambda a: cmd_proactive_check(snapshot_path=a.snapshot_path)
    )

    p_wakeword = sub.add_parser(
        "wake-word", help="Start wake word detection (blocking)."
    )
    p_wakeword.add_argument(
    ...
    p_cost_red = sub.add_parser(
        "cost-reduction", help="Show local vs cloud query ratio and trend."
    )
    p_cost_red.add_argument(
    ...
    p_selftest.add_argument(
        "--threshold", type=float, default=0.5, help="Score threshold for alerts."
    )
