...
_CRITICAL_FUZZY_TARGETS: list[
    tuple[str, Callable[["_DispatchCtx"], tuple[str, int]]]
] = []
...
_DISPATCH_RULES: list[_IntentRule] = [
    ...
    (
        lambda low: (
            ("connect" in low or "setup" in low)
    ...
    (_match_all_any("gaming mode", "status", "state"), _handle_gaming_mode_status),
    # -- Info, web & ops --
    (
        lambda low: (
            ("weather" in low or "forecast" in low) and "my calendar" not in low
        ),
        _handle_weather,
    ),
    (
    ...
    (
        lambda low: (
            low.startswith("call ")
            or "place a call" in low
            or "make a call" in low
            or "phone call" in low
        ),
        _handle_phone_place_call,
    ),
    # -- Ops sync, brief, automation & content generation --
    (_match_all_any("sync", "calendar", "email", "inbox", "ops"), _handle_ops_sync),
    (
        _match_any(
            "daily brief",
            "ops brief",
    ...
    (_match_all_any("automation", "run", "execute", "start"), _handle_automation_run),
    ...
    (
        _match_any(
            "cancel mission", "cancel the mission", "stop mission", "abort mission"
        ),
        _handle_mission_cancel,
    ),
    (
        _match_any(
            "mission status", "learning mission", "active missions", "my missions"
        ),
...
def cmd_voice_run_impl(
    ...
    ctx = _build_dispatch_ctx(
        ...
        repo_root=repo_root,
        respond_fn=_respond,
        require_auth_fn=_require_state_mutation_voice_auth,
        web_augmented_fn=_web_augmented_llm_conversation,
        cmd_fns=cmd_fns,
    )
