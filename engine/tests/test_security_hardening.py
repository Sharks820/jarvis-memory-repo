...
class TestPathTraversalProtection:
    ...
    def test_gaming_state_path_traversal_blocked(
    ...
    def test_gaming_state_symlink_attack_detected(
        self, tmp_path: Path, monkeypatch
    ) -> None:
...
class TestGamingModeSecurity:
    ...
    def test_gaming_mode_auto_detect_process_validation(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            subprocess, "run", lambda *args, **kwargs: SuspiciousTasklist()
        )
...
class TestAutoIngestSecurity:
    ...
    def test_auto_ingest_respects_disable_flag(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        ...
        result = auto_ingest_mod.auto_ingest_memory(
            source="user", kind="episodic", task_id="test", content="Test content"
        )
...
class TestMissionSecurity:
    ...
    def test_mission_url_safety_check_blocks_private_ips(self) -> None:
        ...
        for url in private_urls:
            assert not is_safe_public_url(url), f"Private URL should be blocked: {url}"
    ...
    def test_mission_fetch_respects_max_bytes(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            web_fetch,
            "build_opener",
            lambda *a: type("O", (), {"open": mock_urlopen})(),
        )
