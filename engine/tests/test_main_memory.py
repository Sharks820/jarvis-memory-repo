...
class TestContradictionResolve:
    ...
    def test_resolve_success(self, capsys, mock_bus):
        ...
        result = ContradictionResolveResult(
            success=True, node_id="n1", resolution="accept_new", message="Resolved."
        )
        bus = mock_bus(result)
        rc = main_mod.cmd_contradiction_resolve(
            contradiction_id=1, resolution="accept_new", merge_value=""
        )
        assert rc == 0
        out = capsys.readouterr().out
    ...
    def test_resolve_failure(self, capsys, mock_bus):
        ...
        bus = mock_bus(result)
        rc = main_mod.cmd_contradiction_resolve(
            contradiction_id=99, resolution="keep_old", merge_value=""
        )
        assert rc == 1
        out = capsys.readouterr().out
...
class TestHarvest:
    ...
    def test_harvest_basic(self, capsys, mock_bus):
        ...
        bus = mock_bus(result)
        rc = main_mod.cmd_harvest(
            topic="quantum computing", providers=None, max_tokens=2048
        )
        assert rc == 0
        out = capsys.readouterr().out
...
class TestIngestSession:
    ...
    def test_ingest_session_claude(self, capsys, mock_bus):
        ...
        result = IngestSessionResult(
            source="claude", sessions_processed=5, records_created=12, return_code=0
        )
        bus = mock_bus(result)
        rc = main_mod.cmd_ingest_session(
            source="claude", session_path=None, project_path=None
        )
        assert rc == 0
        out = capsys.readouterr().out
    ...
    def test_ingest_session_with_path(self, capsys, mock_bus):
        ...
        result = IngestSessionResult(
            source="codex", sessions_processed=1, records_created=4, return_code=0
        )
        bus = mock_bus(result)
        rc = main_mod.cmd_ingest_session(
            source="codex", session_path="/tmp/session.json", project_path=None
        )
        assert rc == 0
    ...
    def test_budget_status(self, capsys, mock_bus):
        ...
        result = HarvestBudgetResult(
            summary={
                ...
                "total_cost_usd": 0.15,
                "providers": [
                    {"provider": "groq", "total_cost_usd": 0.10, "total_requests": 50}
                ],
            },
            return_code=0,
        )
        bus = mock_bus(result)
        rc = main_mod.cmd_harvest_budget(
            action="status",
            provider=None,
            period=None,
            limit_usd=None,
...
class TestLearn:
    ...
    def test_learn_basic(self, capsys, mock_bus):
        ...
        result = LearnInteractionResult(
            records_created=2, message="Learned 2 patterns."
        )
        bus = mock_bus(result)
        rc = main_mod.cmd_learn(
            user_message="How's the weather?", assistant_response="It's sunny."
        )
        assert rc == 0
        out = capsys.readouterr().out
...
class TestFlagExpired:
    ...
    def test_flag_expired(self, capsys, mock_bus):
        ...
        result = FlagExpiredFactsResult(
            expired_count=7, message="Flagged 7 expired facts."
        )
        bus = mock_bus(result)
...
class TestProactiveCheck:
    ...
    def test_proactive_with_alerts(self, capsys, mock_bus):
        ...
        result = ProactiveCheckResult(
            alerts_fired=1, alerts=alerts_list, message="1 alert triggered."
        )
        bus = mock_bus(result)
...
class TestBrainCompact:
    ...
    def test_brain_compact_text(self, capsys, mock_bus):
        ...
        result = BrainCompactResult(
            result={"compacted": True, "removed": 50, "kept": 1800}
        )
        bus = mock_bus(result)
...
class TestBrainRegression:
    ...
    def test_brain_regression_text(self, capsys, mock_bus):
        ...
        result = BrainRegressionResult(
            report={"status": "healthy", "duplicate_ratio": 0.02}
        )
        bus = mock_bus(result)
...
class TestBrainContext:
    ...
    def test_brain_context_empty_query(self, capsys, monkeypatch):
        rc = main_mod.cmd_brain_context(
            query="   ", max_items=5, max_chars=1200, as_json=False
        )
        assert rc == 2
        out = capsys.readouterr().out
        assert "error" in out
                ...
                "selected": [
                    {
                        ...
                        "source": "user",
                        "kind": "semantic",
                        "summary": "Gaming modes...",
                    }
                ],
                "canonical_facts": [
                    {"key": "mode", "value": "gaming", "confidence": 0.9}
                ],
            }
        )
        bus = mock_bus(result)
        rc = main_mod.cmd_brain_context(
            query="gaming", max_items=5, max_chars=1200, as_json=True
        )
        assert rc == 0
        parsed = json.loads(capsys.readouterr().out)
    ...
    def test_brain_context_text_output(self, capsys, mock_bus):
        ...
        bus = mock_bus(result)
        rc = main_mod.cmd_brain_context(
            query="test", max_items=5, max_chars=1200, as_json=False
        )
        assert rc == 0
        out = capsys.readouterr().out
...
class TestBrainStatus:
    ...
    def test_brain_status_json(self, capsys, mock_bus):
        ...
        result = BrainStatusResult(
            status={"updated_utc": "2026-01-01", "branch_count": 5, "branches": []}
        )
        bus = mock_bus(result)
...
class TestIngestCommand:
    ...
    def test_ingest_basic(self, capsys, mock_bus):
        ...
        result = IngestResult(
            record_id="rec-123", source="user", kind="semantic", task_id="t1"
        )
        bus = mock_bus(result)
        rc = main_mod.cmd_ingest(
            source="user", kind="semantic", task_id="t1", content="Test content"
        )
        assert rc == 0
        out = capsys.readouterr().out
...
class TestRunTask:
    ...
    def test_run_task_denied(self, capsys, mock_bus):
        ...
        result = RunTaskResult(
            allowed=False, reason="privileged task denied", return_code=2
        )
        bus = mock_bus(result)
        rc = main_mod.cmd_run_task(
            task_type="video",
...
class TestMemorySnapshotEdgeCases:
    ...
    def test_snapshot_verify_ok(self, capsys, mock_bus):
        ...
        bus = mock_bus(result)
        rc = main_mod.cmd_memory_snapshot(
            create=False, verify_path="/tmp/snap.zip", note=""
        )
        assert rc == 0
        out = capsys.readouterr().out
    ...
    def test_snapshot_create(self, capsys, mock_bus):
        ...
        result = MemorySnapshotResult(
            ...
            metadata_path="/tmp/snap.meta.json",
            signature_path="/tmp/snap.sig",
            sha256="abc123",
            file_count=10,
        )
        bus = mock_bus(result)
...
class TestSelfHealMocked:
    ...
    def test_self_heal_json(self, capsys, mock_bus):
        ...
        report = {
            "status": "ok",
            "actions": ["checked_db", "verified_config"],
            "regression": {
                "status": "healthy",
                "duplicate_ratio": 0.0,
                "unresolved_conflicts": 0,
...
class TestMigrateMemory:
    ...
    def test_migrate_success(self, capsys, mock_bus):
        ...
        result = MigrateMemoryResult(
            summary={
                "totals": {"inserted": 100, "skipped": 5, "errors": 0},
                "db_path": "/tmp/mem.db",
            },
            return_code=0,
        )
