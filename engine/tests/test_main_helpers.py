...
class TestHelperFunctions:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                "Call +14155551234 please", "+14155551234", id="international_format"
            ),
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                "search the web for python asyncio", "python", id="search_web"
            ),
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                "go to https://example.com", "https://example.com", id="https_url"
            ),
            pytest.param(
                "visit www.google.com", "https://www.google.com", id="www_url"
            ),
            pytest.param("no url here", "", id="no_url"),
            pytest.param(
                "x" * 1300 + "https://late.com", "", id="truncation_at_1024_chars"
            ),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ...
                False,
                False,
                id="execute_flag_forces_non_readonly",
            ),
            ...
            pytest.param(
                ...
                False,
                True,
                id="conversational_fallthrough",
            ),
        ],
    )
    def test_is_read_only_voice_request(self, text, execute, approve, expected):
        assert (
            voice_pipeline_mod._is_read_only_voice_request(
    ...
    @pytest.mark.parametrize(
        "content,forbidden_substr",
        [
            pytest.param(
                '{"api_key": "sk-secret123", "data": "normal"}',
                "sk-secret123",
                id="json_api_key",
            ),
            pytest.param(
                "Authorization: bearer sk-my-token-abc",
                "sk-my-token-abc",
                id="bearer_token",
            ),
            pytest.param(
                "master password: ExamplePass123! token=abc123",
                "ExamplePass123!",
                id="master_password",
            ),
        ],
...
class TestAutoIngestMemory:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                None, "invalid_source", "semantic", "Test", id="invalid_source"
            ),
            pytest.param(None, "user", "bogus", "Test", id="invalid_kind"),
            pytest.param(None, "user", "semantic", "", id="empty_content"),
        ],
    )
    def test_auto_ingest_returns_empty(
        self, monkeypatch, tmp_path, env_disable, source, kind, content
    ):
