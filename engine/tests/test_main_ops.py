...
def test_cmd_daemon_run_uses_active_interval_when_active(
...
def test_cmd_daemon_run_skips_autopilot_while_gaming_mode_enabled(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(main_mod, "repo_root", lambda: tmp_path)
    monkeypatch.setattr(daemon_loop_mod, "repo_root", lambda: tmp_path)
...
def test_cmd_daemon_run_skips_autopilot_when_auto_detect_finds_game(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(main_mod, "repo_root", lambda: tmp_path)
    monkeypatch.setattr(daemon_loop_mod, "repo_root", lambda: tmp_path)
    ...
    monkeypatch.setattr(
        ...
        lambda: (True, "fortniteclient-win64-shipping.exe"),
    )
...
def test_cmd_daemon_run_skips_autopilot_when_runtime_paused(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(main_mod, "repo_root", lambda: tmp_path)
    monkeypatch.setattr(daemon_loop_mod, "repo_root", lambda: tmp_path)
...
def test_cmd_daemon_run_safe_mode_forces_non_execute_cycle(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(main_mod, "repo_root", lambda: tmp_path)
    monkeypatch.setattr(daemon_loop_mod, "repo_root", lambda: tmp_path)
...
class TestOpsBrief:
    ...
    def test_ops_brief_basic(self, capsys, mock_bus):
        ...
        result = OpsBriefResult(
            brief="Good morning, Conner. Here's your brief.", saved_path=""
        )
        bus = mock_bus(result)
        rc = main_mod.cmd_ops_brief(
            snapshot_path=Path("/tmp/snap.json"), output_path=None
        )
        assert rc == 0
    ...
    def test_ops_brief_with_save(self, capsys, mock_bus):
        ...
        bus = mock_bus(result)
        rc = main_mod.cmd_ops_brief(
            snapshot_path=Path("/tmp/snap.json"), output_path=Path("/tmp/brief.txt")
        )
        assert rc == 0
        out = capsys.readouterr().out
...
class TestOpsExportActions:
    ...
    def test_export_actions(self, capsys, mock_bus):
        ...
        result = OpsExportActionsResult(
            actions_path="/tmp/actions.json", action_count=3
        )
        bus = mock_bus(result)
        rc = main_mod.cmd_ops_export_actions(
            snapshot_path=Path("/tmp/snap.json"), actions_path=Path("/tmp/actions.json")
        )
        assert rc == 0
        out = capsys.readouterr().out
...
class TestAutomationRun:
    ...
    def test_automation_run_basic(self, capsys, mock_bus):
        ...
        bus = mock_bus(result)
        rc = main_mod.cmd_automation_run(
...
class TestMissionCreate:
    ...
    def test_create_success(self, capsys, mock_bus):
        ...
        bus = mock_bus(result)
        rc = main_mod.cmd_mission_create(
            topic="Python async", objective="", sources=["google", "reddit"]
        )
        assert rc == 0
        out = capsys.readouterr().out
        ...
        missions = [
            {
                ...
                "status_detail": "Finalized report",
            },
            {
                "mission_id": "m2",
                "status": "running",
                "topic": "Realtime STT",
                "verified_findings": 1,
                "updated_utc": "2026-01-02",
                "progress_pct": 45,
                "status_detail": "Scanning 8 pages",
            },
        ]
...
class TestMissionRun:
    ...
    def test_run_success(self, capsys, mock_bus):
        ...
        bus = mock_bus(result)
        rc = main_mod.cmd_mission_run(
            mission_id="m1", max_results=8, max_pages=12, auto_ingest=True
        )
        assert rc == 0
        out = capsys.readouterr().out
    ...
    def test_run_failure(self, capsys, mock_bus):
        ...
        bus = mock_bus(result)
        rc = main_mod.cmd_mission_run(
            mission_id="bad", max_results=8, max_pages=12, auto_ingest=True
        )
        assert rc == 2
...
class TestGrowthReport:
    ...
    def test_report(self, capsys, mock_bus):
        ...
        result = GrowthReportResult(
            summary={
                "runs": 10,
                "latest_model": "gemma3:4b",
                "latest_score_pct": 80.0,
                "delta_vs_prev_pct": 2.5,
                "window_avg_pct": 78.0,
                "latest_ts": "2026-01-01",
            }
        )
        bus = mock_bus(result)
...
class TestGrowthAudit:
    ...
    def test_audit(self, capsys, mock_bus):
        ...
        bus = mock_bus(result)
        rc = main_mod.cmd_growth_audit(
            history_path=Path("/tmp/hist.jsonl"), run_index=-1
        )
        assert rc == 0
        out = capsys.readouterr().out
...
class TestIntelligenceDashboard:
    ...
    def test_dashboard_json(self, capsys, monkeypatch, mock_bus, tmp_path):
        ...
        dashboard = {
            ...
            "jarvis": {
                "score_pct": 80.0,
                "delta_vs_prev_pct": 1.0,
                "window_avg_pct": 78.0,
                "latest_model": "gemma3:4b",
            },
            "methodology": {
                "history_runs": 10,
                "slope_score_pct_per_run": 0.5,
        ...
        rc = main_mod.cmd_intelligence_dashboard(
            last_runs=20, output_path="", as_json=True
        )
        assert rc == 0
    ...
    def test_dashboard_text(self, capsys, monkeypatch, mock_bus, tmp_path):
        ...
        dashboard = {
            ...
            "etas": [
                {
                    "target_name": "GPT-4",
                    "target_score_pct": 90.0,
                    "eta": {"status": "on_track", "runs": 20, "days": 60},
                }
            ],
        ...
        monkeypatch.setattr(bus_mod, "repo_root", lambda: tmp_path)
        rc = main_mod.cmd_intelligence_dashboard(
            last_runs=20, output_path="", as_json=False
        )
        assert rc == 0
        out = capsys.readouterr().out
...
class TestIntelligenceDashboardOutputPath:
    ...
    def test_output_path_outside_repo_json(
        self, capsys, monkeypatch, mock_bus, tmp_path
    ):
        from jarvis_engine.commands.ops_commands import IntelligenceDashboardResult
        ...
        rc = main_mod.cmd_intelligence_dashboard(
            last_runs=5,
            output_path="/tmp/totally/outside/dashboard.json",
            as_json=True,
        )
        assert rc == 2
        out = capsys.readouterr().out
    ...
    def test_output_path_inside_repo_json(
        self, capsys, monkeypatch, mock_bus, tmp_path
    ):
        from jarvis_engine.commands.ops_commands import IntelligenceDashboardResult
        ...
        rc = main_mod.cmd_intelligence_dashboard(
            last_runs=5, output_path=out_path, as_json=True
        )
        assert rc == 0
...
class TestDaemonSelfTest:
    ...
    def _run_daemon_impl(
        self, tmp_path, monkeypatch, *, self_test_every_cycles=1, max_cycles=1
    ):
    ...
    def test_self_test_runs_at_correct_cycle(self, capsys, monkeypatch, tmp_path):
        ...
        with patch(
            ...
            rc = self._run_daemon_impl(
                tmp_path, monkeypatch, self_test_every_cycles=1, max_cycles=1
            )

        assert rc == 0
        mock_cls.assert_called_once_with(mock_engine, mock_embed, score_threshold=0.5)
    ...
    def test_self_test_skipped_when_disabled(self, capsys, monkeypatch, tmp_path):
        ...
        with patch("jarvis_engine.proactive.self_test.AdversarialSelfTest") as mock_cls:
            rc = self._run_daemon_impl(
                tmp_path, monkeypatch, self_test_every_cycles=0, max_cycles=1
            )
