...
- 2026-03-07: consolidated quality/platform hardening PR: (1) Android chat submission hardened with atomic send lock (`compareAndSet`), draft restored on submit failure, and composer disabled while sending; (2) CI pipeline established (lint/bandit/pytest/smoke gates); (3) comprehensive smoke test suite added (261 tests across 25 sections covering module imports, memory, KG, learning, security, intent routing, integration, performance, and property-based invariants); (4) CONTRIBUTING.md and CODEOWNERS added; (5) AGENTS.md enhanced with quality gates, module ownership, and anti-patterns. Supersedes PR #18 and PR #19.
- 2026-03-07: **security bug fix** — injection firewall base64 detection threshold was 50 chars; lowered to 16 chars because `base64("ignore all previous instructions")` is only 44 chars total. Added hex-blob decode-and-check layer. All three encoding vectors (base64, hex, URL-percent) now tested in smoke suite. README.md completely redesigned as world-class main page with ToC, tables, and all sections organized for easy forward updates. CONTRIBUTING.md and AGENTS.md expanded to be fully comprehensive with encoding-aware security guidance.
