...
namespace Jarvis.EditorBridge
{
    ...
    public static class JarvisEditorBridge
    {
        ...
        /// JARVIS_BRIDGE_SECRET environment variable; falls back to reading
        /// a runtime-generated secret file shared with the Python side.
        /// Never falls back to a weak default.
        /// </summary>
        internal static readonly string SharedSecret = GetSharedSecret();

        private static string GetSharedSecret()
        {
            string envSecret = Environment.GetEnvironmentVariable("JARVIS_BRIDGE_SECRET");
            if (!string.IsNullOrEmpty(envSecret)) return envSecret;
            // Read from runtime file (generated by Python side).
            // Search multiple candidate locations: the Jarvis repo root (via env var),
            // the Unity project root, and common user-home paths.
            var candidates = new List<string>();
            string repoRoot = Environment.GetEnvironmentVariable("JARVIS_REPO_ROOT");
            if (!string.IsNullOrEmpty(repoRoot))
                candidates.Add(System.IO.Path.Combine(repoRoot, ".planning", "runtime", "bridge_secret.txt"));
            candidates.Add(System.IO.Path.Combine(
                Application.dataPath, "..", ".planning", "runtime", "bridge_secret.txt"));
            string userProfile = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
            candidates.Add(System.IO.Path.Combine(
                userProfile, "jarvis-memory-repo", ".planning", "runtime", "bridge_secret.txt"));
            foreach (string secretPath in candidates)
            {
            ...
            // No secret available — require explicit configuration
            Debug.LogWarning(
                "[Jarvis] No JARVIS_BRIDGE_SECRET set and no runtime secret file found. " +
                "Bridge authentication will reject all connections. " +
                "Set JARVIS_BRIDGE_SECRET env var or ensure .planning/runtime/bridge_secret.txt exists.");
            return "";
        }
        ...
        static JarvisEditorBridge()
        {
            ...
            // Register lifecycle callbacks (unsubscribe first to prevent accumulation)
